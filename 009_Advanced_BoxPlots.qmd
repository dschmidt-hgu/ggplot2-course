...
* you have seen some alternative plotting types to be considered for showing data distributions that include more/encode differen information than a standard boxplot
* you understand why reordering of categories can make sense when comparing groups 


**... tool usage:**

* you can follow a `ggplot2` code from code extensions related to boxplot alternatives  
...
pacman::p_load("ggrain")    #raincloudplots
```
...
# Simulate Data
set.seed(1)
data_normal <- tibble(x="obs",y=rnorm(n=10000,mean=0,sd=1))
data_exponential <- tibble(x="obs",y=rexp(n=10000,rate=1))
data_uniform <- tibble(x="obs",y=runif(n=10000,min=0,max=1))
...
clim_lt <- read_csv("clim_DWD_Geisenheim_historic_daily.csv",show_col_types = FALSE)

clim_lt <- clim_lt %>% mutate("Month"=factor(month(MESS_DATUM)))
