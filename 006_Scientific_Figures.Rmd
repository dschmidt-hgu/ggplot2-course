...
#color blind safe: https://colorbrewer2.org/#type=diverging&scheme=RdYlBu&n=6
#       mean, min, max
cols <- c("#fc8d59","#4575b4","#d73027")

ggplot(m_minmaxmeanTDF, aes(x=Year,y=value, fill=variable))+      #
  geom_point(shape=21,alpha=0.5)+                                 # shape=21 can be filled
  geom_smooth(method="lm",aes(color=variable))+                   # lineare trendline, color
  ...
  scale_y_continuous(name="T (°C)",breaks=seq(-10,30,by=5))+      # Modify y axis | more breaks for differentiation within group
  coord_fixed(ratio = 3)+                                         # Select aspect ratio | not too stretched
  geom_text(data=m_minmaxmeanTDF[Year==max(Year)],                # annotate with geom text to use data
              aes(y=value, x=Year+2, label=c("Mean","Min","Max"), # custom labels | !!better introduce a label variable - fail safe
              color=variable),                                    # also assign color
              hjust=0,                                            # horizontal align = left
              size=3)+                                            # adjust size
  scale_x_continuous(expand = expansion(mult = c(0.05, 0.15)))+   # expand x axis to include text
  scale_color_manual(values=cols)+                                # color blind safe: https://colorbrewer2.org/#type=diverging&scheme=RdYlBu&n=6
  scale_fill_manual(values=cols)                                  # same as color
  #annotate(geom="text",y=m_minmaxmeanTDF[Year==max(Year)]$value, x=m_minmaxmeanTDF[Year==max(Year)]$Year+2, label=c("Mean","Min","Max"))

# one could also use - differences to first date
